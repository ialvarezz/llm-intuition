...
Underneath the protocol is a small, fixed vocabulary. The specification defines what a server is allowed to offer a client, and it's exactly three things: "Servers offer any of the following features to clients:" **Resources** are "Context and data, for the user or the AI model to use"; **Prompts** are "Templated messages and workflows for users"; and **Tools** are "Functions for the AI model to execute" [S2].
