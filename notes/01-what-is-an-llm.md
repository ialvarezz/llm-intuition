...
But a model trained this way — called a **base model** — is still just a document continuer. Karpathy's example: prompt it with "It's raining so I should bring an" and it continues with "umbrella," not because it's answering your question but because that's the statistically likely continuation of that sentence in the kind of text it was trained on [S1]. A base model won't reliably answer "What's the capital of France?" — it's just as likely to continue your prompt with more questions in the same style, because that's a common pattern after a question in training data.
...
3Blue1Brown calls this exact cycle "predict, sample, repeat," and notes it's literally what produces the token-by-token appearance of ChatGPT typing out an answer in real time [S2].
