...
- Softmax normalization (verbatim, added 2026-07-23 — Step 3 gap-check for slide 9's normalization claim, see dated note below): "We compute the dot products of the query with all keys, divide each by $\sqrt{d_k}$, and apply a softmax function to obtain the weights on the values." Formula, verbatim: "Attention(Q, K, V) = softmax(QKᵀ / √d_k)V". This is the direct source for the mechanism behind "attention weights are normalized across positions": softmax takes the raw compatibility scores between one query and every key in the sequence and converts them into a set of weights over the values — softmax's defining property (not separately stated as a formula-adjacent sentence in this paper, but standard/undisputed for the softmax function it names) is that its outputs sum to 1 across whatever set of scores it's applied to. Applied here across all key positions for a given query, this means the weight assigned to any one position is directly reduced by however many other positions are competing for a share of that same fixed total — more candidate positions (longer context) means the same "one unit" of attention gets split more ways, so each individual token's share shrinks on average as sequence length n grows, all else equal.

### Quote audit (2026-07-21; softmax quote added and audited 2026-07-23)
...
- **2026-07-23 addition:** ran `grep -i "softmax\|normaliz" research/context-reasoning/03-attention-curve.md` per the brief's Step 3 — found no existing coverage of softmax/normalization in this dump (only unrelated occurrences of the word "normalized" inside other quote-audit sentences describing the audit process itself, not a sourced claim about attention). Genuine gap confirmed. Re-fetched https://arxiv.org/html/1706.03762v7 directly via curl (raw HTML, not the WebFetch summarizer), stripped tags/scripts and collapsed whitespace with a Python script, and located the softmax sentence and formula in Section 3.2.1 "Scaled Dot-Product Attention." Both the sentence "We compute the dot products of the query with all keys, divide each by ... and apply a softmax function to obtain the weights on the values." (math notation rendered as plain-text $\sqrt{d_k}$ in the dump to avoid MathML-rendering artifacts from the live page's HTML) and the formula "Attention(Q, K, V) = softmax(QKᵀ / √d_k)V" confirmed present in the live page (formula transcribed from the page's MathML rendering, which uses stacked/spaced glyphs rather than plain-text — content and structure match exactly). CONFIRMED.
...
- **2026-07-23 addition:** per this task's Step 3, grepped this dump for `softmax|normaliz` before fetching anything new — found a genuine gap (no prior coverage). Fetched the softmax-normalization sentence and formula from Vaswani et al. (already cited as [S3] in this dump) and appended it to the existing [S3] block above, rather than creating a new source entry, since it's the same paper/URL already in the dump.
